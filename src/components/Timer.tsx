...
function Timer() {
  ...
  const { secondsRemaining, circleDash } = state;
  ...
  return (
    <div
      className={`base-timer relative h-[100px] w-[100px] py-1 px-1 rounded-full bg-orange-100 shadow-lg`}
    >
      <svg
        className="base-timer__svg -scale-x-100"
        viewBox="0 0 100 100"
        xmlns="http://www.w3.org/2000/svg"
      >
        <g className="base-timer__circle fill-orange-100 stroke-none drop-shadow-lg">
          <circle
            className="base-timer__path-elapsed stroke-[4px] stroke-orange-50"
            cx="50"
            cy="50"
            r="45"
          />
      ...
      <span
        className={`absolute top-0 left-0 h-[100px] w-[100px] flex items-center justify-center ${secondsRemaining < 5 && "animate-pulse"}`}
      >
        {mins < 10 && "0"}
        {mins}:{seconds < 10 && "0"}
        {seconds}
